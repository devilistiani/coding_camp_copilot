...
def load_data():
    ...
    df_bersih = pd.read_csv(base_dir / "data" / "data_bersih.csv")
    df_ab_test = pd.read_csv(base_dir / "data" / "hasil_ab_testing.csv")
    df_tfidf = pd.read_csv(base_dir / "data" / "dataset_tfidf.csv")

    df_bersih = df_bersih.rename(
        columns={"Kategori_Bersih": "Path"}
    )

    df_bersih = df_bersih[
        ~df_bersih["Path"].str.strip().str.lower().isin(
