...
export function Chat() {
  ...
  useEffect(() => {
    ...
    if (conversationId === queryConvId) return;
    ...
    (async () => {
      try {
        const res = await authFetch(`${API_BASE}/conversations/${queryConvId}`, {
        ...
        if (!res.ok) {
          setMessages([{
            id: nextId(),
            role: "bot",
            content: json?.error?.message ?? "Gagal memuat percakapan.",
            timestamp: new Date(),
            category: "Umum",
            urgency: "low",
            confidence: 0,
        ...
        const rawMessages = (data?.messages ?? []) as Array<{
          ...
          ai_metadata: { all_scores?: Record<string, number> } | null;
          created_at: string;
        }>;
        const mapped: Message[] = rawMessages.map((m) => {
          const allScores = (m.ai_metadata?.all_scores ?? {}) as Record<string, number>;
          const sources = Object.keys(allScores)
            .sort((a, b) => (allScores[b] ?? 0) - (allScores[a] ?? 0))
            .slice(0, 2);
          return {
            id: m.id,
            ...
            category: (m.category ?? undefined) as CategoryType | undefined,
            urgency: (m.urgency ?? undefined) as UrgencyLevel | undefined,
            confidence: m.confidence ?? undefined,
            draftReply: m.draft_reply ?? undefined,
        ...
        if (!aborted) {
          setMessages([{
            id: nextId(),
            role: "bot",
            content: "Server tidak dapat dijangkau. Coba lagi nanti.",
            timestamp: new Date(),
            category: "Umum",
            urgency: "low",
            confidence: 0,
    ...
    return () => { aborted = true; };
  }, [queryConvId, conversationId, authFetch]);
  ...
  const callAPI = useCallback(async (question: string): Promise<ApiResponse> => {
    ...
    const ai = data?.ai_message ?? {};
    const meta = ai.ai_metadata ?? {};
    const allScores = (meta.all_scores ?? {}) as Record<string, number>;
    const sources = Object.keys(allScores)
      .sort((a, b) => (allScores[b] ?? 0) - (allScores[a] ?? 0))
      .slice(0, 2);
    ...
    return {
      answer: ai.content ?? "",
      category: (ai.category ?? "Umum") as CategoryType,
      urgency: (ai.urgency ?? "low") as UrgencyLevel,
      confidence: ai.confidence ?? 0,
      draft_reply: ai.draft_reply ?? undefined,
      sources,
    };
  ...
  const sendMessage = async (text?: string) => {
    ...
    try {
      ...
      setMessages(p => [...p, {
        id: nextId(),
        role: "bot",
        content: "Maaf, server sedang tidak dapat dijangkau. Coba lagi sebentar.",
        timestamp: new Date(),
        category: "Umum",
        urgency: "low",
        confidence: 0,
      ...
      <div className="flex-1 flex flex-col min-h-0 min-w-0">
        ...
        <div ref={messagesContainerRef} className="flex-1 overflow-y-auto px-4 py-6 scroll-smooth">
          <div className="max-w-3xl mx-auto w-full space-y-2">
            ...
            <AnimatePresence>
              {showSuggestions && !loadingConversation && (
                <motion.div
